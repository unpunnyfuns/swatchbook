/**
 * Dispatch half of `normalizePermutations` — selects between resolver,
 * layered, and plain-parse loaders based on config shape. Uses a real
 * temp workspace per test.
 *
 * The gating half (mutually-exclusive config validation, no workspace)
 * lives in `permutations-normalize-gating.test.ts`.
 */
