...
/**
 * Pure helper — exported so consumers driving their own Tailwind
 * integration (outside the addon) can use the same body the virtual
 * module serves.
 */
export function renderTailwindTheme(
  project: Project,
  roles: Readonly<Record<string, readonly (readonly [string, string])[]>> = DEFAULT_ROLES,
): string {
  ...
  return [
    '/* Generated by @unpunnyfuns/swatchbook-integrations/tailwind.',
    ' * Imported via `virtual:swatchbook/tailwind.css` — do not edit. */',
    "@import 'tailwindcss';",
    '',
    '@theme {',
    ...entries,
...
/**
 * Curated default role map covering the common DTCG semantic roles
 * from the reference fixture. Consumers with divergent paths can pass
 * their own map through `tailwindIntegration({ roles })`.
 */
export const DEFAULT_ROLES: Readonly<Record<string, readonly (readonly [string, string])[]>> = {
  color: [
