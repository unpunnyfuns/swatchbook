// get_token / get_css_usage must report the CSS var name plugin-css
...
it('get_css_usage reports the Terrazzo-derived var name for camelCase paths', async () => {
  const mcp = await startTestServer(await camelCaseProject());
  try {
    const result = await mcp.callJson<{ cssVar: string }>('get_css_usage', {
      path: 'color.brandPrimary',
    });
    expect(result.cssVar).toBe('var(--t-color-brand-primary)');
  } finally {
