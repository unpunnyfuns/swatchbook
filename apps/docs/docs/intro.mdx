...
- **[Reference](./reference/addon.mdx)** — API surface for each published package, the axis model, the token-pipeline mechanism, the config shape, and the [diagnostics catalog](./reference/diagnostics.mdx) when an error fires.
