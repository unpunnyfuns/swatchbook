...
- **Concepts** — the mental model: [why axes, not themes](./concepts/axes-vs-themes.mdx), [theming inputs](./concepts/theming-inputs.mdx), [axes](./concepts/axes.mdx), [presets](./concepts/presets.mdx), [the token pipeline](./concepts/token-pipeline.mdx).
