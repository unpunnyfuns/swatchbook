...
- **`parser: Could not resolve alias "{color.palette.blue.500}"`** — the alias target doesn't exist in the merged graph. Check for a typo in the path, or a missing ref file that wasn't included in `tokens` globs.
