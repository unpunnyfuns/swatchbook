...
See [consuming the active theme](../guides/consuming-the-active-theme.mdx) for how individual stories react to axis changes, which is a related but distinct plumbing.
...
Tokens reach the blocks through a Vite virtual module the addon publishes — no generated files, no separate build step, no config drift. Edit a token, see the preview repaint. For production CSS, run [Terrazzo](https://terrazzo.app/)'s CLI against the same DTCG sources.
