---
id: consuming-the-active-permutation
title: Consuming the active permutation
sidebar_position: 5
---

# Consuming the active permutation
...
```tsx
import { useActiveAxes, useActivePermutation, useColorFormat } from '@unpunnyfuns/swatchbook-addon';
...
`useActivePermutation()` gives the composed tuple string (`"Dark · Brand A · Normal"`); `useColorFormat()` gives the active color-format selection. Same shape, same context.
