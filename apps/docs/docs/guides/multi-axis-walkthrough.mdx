...
- **Demo components** in the reference Storybook (Button, Card, Input) read `var(--ds-color-sys-*)` directly and repaint as you switch axes — this is the documentation affordance that lets authors verify tokens in every context, not a production theming contract.
