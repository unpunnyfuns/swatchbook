...
`SwatchbookProvider`, `useSwatchbookData`, `useActivePermutation`, `useActiveAxes`, `useColorFormat`, and the backing contexts (`SwatchbookContext`, `PermutationContext`, `AxesContext`, `ColorFormatContext`) live in `@unpunnyfuns/swatchbook-blocks`. `@unpunnyfuns/swatchbook-addon` re-exports them verbatim, so either import path works depending on which packages your consumer already pulls in.
...
For authoring custom blocks against the same provider the built-ins read from, see the [**Hooks reference**](./hooks.mdx) — `useSwatchbookData`, `useActivePermutation`, `useActiveAxes`, `useColorFormat`.
