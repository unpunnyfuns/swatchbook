...
`loadProject` enumerates one tuple per non-default `(axis, context)` pair plus the default tuple — the count grows with axis cardinality, not with every possible combination. A resolver with 20 axes × 5 contexts loads around 80 tuples regardless of whether the full combination space is hundreds or millions. The toolbar surfaces every axis; the CSS emitter writes per-axis cell blocks plus compound blocks only for tokens with genuine joint divergence; runtime CSS composition lands the right value at any multi-axis selector.

The loader never materializes the full combination space. Two scoping levers remain, both presentational:
