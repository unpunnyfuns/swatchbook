...
Published as `@unpunnyfuns/swatchbook-core`. Framework-free DTCG loader. Parses token files via Terrazzo, builds a walkable token graph, resolves aliases per tuple. No React, no Storybook dependency.
...
| `getVariance` | `(graph, path) → AxisVarianceResult` | `AxisVarianceResult` for one token path — which axes affect it, and the stringified value per context per axis. |
...
Per-token-path node. `baselineValue` is the resolved leaf at the default tuple. `writes[axisName][contextName]` holds per-(axis, context) overlay declarations. `affectedBy` is the precomputed axis-sensitivity index — the set of axes whose writes can change this token's resolved value, used by the walker's fast-path to skip resolution for constant tokens.
...
Return type of [`getVariance(graph, path)`](#graph-queries). Discriminated on `kind`; the cardinality of `varyingAxes` is reflected in the tuple type so consumers narrow exhaustively. The `single` variant adds a convenience `axis: string` accessor.
