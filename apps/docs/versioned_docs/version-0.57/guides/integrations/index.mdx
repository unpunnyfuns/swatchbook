...
Implement the `SwatchbookIntegration` contract from `@unpunnyfuns/swatchbook-core`, export a factory from your package, drop the factory's result into `integrations[]`. Mechanism sketched in the [architecture doc](../../developers/architecture.mdx).
