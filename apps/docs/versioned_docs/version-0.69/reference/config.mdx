...
See [Aligning with your token build](../guides/sharing-terrazzo-options.mdx) for the end-to-end shared-options pattern.
...
See [Sharing Terrazzo options](../guides/sharing-terrazzo-options.mdx) for the pattern of sharing one options file between `terrazzo.config.ts` and `swatchbook.config.ts`.
...
- [Axes reference](../concepts/axes.mdx): the runtime model for selection.
