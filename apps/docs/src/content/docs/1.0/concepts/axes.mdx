---
title: Axes
slug: "1.0/concepts/axes"
sidebar:
  order: 5
---

import AxisFlipDiagram from '#/components/diagrams/AxisFlipDiagram.tsx';
...
A resolver may declare more modifiers than a given Storybook wants to surface. List the axis name in `config.disabledAxes` and the project behaves as if that axis didn't exist for this session: no toolbar dropdown, no extra CSS blocks, the axis pinned to its `default` context in the active tuple. The Design Tokens panel shows a faint pinned indicator so the suppression isn't invisible. See the [`disabledAxes`](/reference/config#disabledaxes) field for details.

## See also

- [Config reference](/reference/config): the `resolver` / `axes` / `default` / `disabledAxes` / `presets` fields.
