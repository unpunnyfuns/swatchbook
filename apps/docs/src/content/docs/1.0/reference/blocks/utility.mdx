...
slug: "1.0/reference/blocks/utility"
sidebar:
  order: 5
---
...
`group` is namespaced by feature area (`swatchbook/<area>`) and is stable across releases, so consumers can filter on it:
...
The [Diagnostics reference](/reference/diagnostics) catalogs what fires under each group.
