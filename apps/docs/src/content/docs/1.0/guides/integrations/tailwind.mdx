---
title: Tailwind
slug: "1.0/guides/integrations/tailwind"
sidebar:
  order: 2
---
...
At runtime the toolbar flips one `data-sb-<axis>` attribute per axis on `<html>` (e.g. `data-sb-mode`), the per-tuple stylesheet redefines `--sb-*` vars, and every Tailwind utility re-paints via cascade.
