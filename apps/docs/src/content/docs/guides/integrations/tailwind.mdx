...
`@unpunnyfuns/swatchbook-integrations/tailwind` aliases [Tailwind v4](https://tailwindcss.com/docs/installation) utility scales to your DTCG tokens. Classes like `bg-sb-surface-default` / `p-sb-md` / `rounded-sb-lg` resolve through the same `var(--sb-*)` chain the swatchbook toolbar flips.
...
At runtime, flipping the toolbar repaints every Tailwind utility through the CSS cascade; see [Consuming the active theme](/guides/consuming-the-active-theme) for the mechanism.
...
With no configuration, the integration walks your project's default-theme token graph at render time and classifies each token into a Tailwind scale by `$type` and path:
...
**Tailwind's `dark:` variant doesn't respond to the toolbar.** Tailwind looks for `.dark` on an ancestor, or `data-theme="dark"` in attribute mode. Swatchbook instead writes the active mode to `data-<cssVarPrefix>-mode` (`data-sb-mode="Dark"` under this guide's `sb` prefix; `data-swatch-mode` by default), so the shape and name don't match. Either point Tailwind at swatchbook's attribute, swapping `sb` for your own prefix:

```css
@custom-variant dark (&:where([data-sb-mode=Dark], [data-sb-mode=Dark] *));
```
