...
**Tailwind's `dark:` variant doesn't respond to the toolbar.** Tailwind looks for `.dark` on an ancestor, or `data-theme="dark"` in attribute mode. swatchbook writes `data-sb-mode="Dark"` instead, so the shape and name don't match. Either point Tailwind at swatchbook's attribute:
