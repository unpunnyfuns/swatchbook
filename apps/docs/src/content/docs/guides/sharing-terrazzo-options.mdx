...
/** Lint rules shared by `tz build` and swatchbook's `<Diagnostics />`. */
...
| [`lintOptions`](/reference/config#lintoptions)         | Terrazzo's lint pass             | which lint rules run during swatchbook's internal parse                                |
...
`lintOptions` reaches swatchbook's lint pass on the layered and plain-parse configurations. A `resolver`-backed project, as in the example above, loads through Terrazzo's resolver rather than `parse()`, and no lint pass runs there; see [`lintOptions`](/reference/config#lintoptions). Sharing the block still keeps the two sides from drifting apart once that changes.
...
If you don't run a production token build alongside swatchbook (the tokens exist only for the docs site and Storybook), leave `cssOptions`, `lintOptions`, `listingOptions`, and `terrazzoPlugins` unset. swatchbook loads `plugin-css` with defaults and `plugin-token-listing` with a single `css` platform. Drift isn't a concern when there's only one pipeline.

## See also

- [Config reference: `cssOptions` / `lintOptions` / `listingOptions` / `terrazzoPlugins`](/reference/config): the prop-by-prop contract.
