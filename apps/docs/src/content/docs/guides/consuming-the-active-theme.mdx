...
import ThemeConsumptionDiagram from '#/components/diagrams/ThemeConsumptionDiagram.tsx';

Swatchbook's toolbar flips the active tuple, and a story rendering under it needs a way to notice the change. Three approaches, from least code to most:

1. **CSS variables**: no code at all, and the right choice for any component that already reads tokens through `var(--…)`.
2. **The React hook `useActiveAxes()`**: for story code inside the Storybook preview, reading the same React context the blocks and decorators use.
3. **DOM observation**: the framework-agnostic fallback for Vue / Angular / Svelte / plain HTML, or React trees outside the preview's provider.

<ThemeConsumptionDiagram />
...
For CSS-in-JS libraries whose theme values can be `var(--…)` strings (styled-components, emotion with a `var(...)`-based theme), see the [CSS-in-JS integration](/guides/integrations/css-in-js); it publishes a ready-made theme accessor without either of the paths below.
...
Inside the preview, prefer `useActiveAxes()`: it updates without the observer cost and pools with the blocks' own re-renders.
...
**Component library theme is a plain JS object: can it read the `--<prefix>-*` variables directly?**
Only if the library consumes CSS variables natively. When the theme is a JS object _read_ at render time (MUI's `createTheme`, Chakra, Mantine), it can't; rebuild the theme object on axis flips with `useActiveAxes()` in React, or the DOM-observation path elsewhere.

**Tailwind's `dark:` variant isn't responding to swatchbook.**
See the [Tailwind integration](/guides/integrations/tailwind#gotchas) for keying Tailwind's dark variant on swatchbook's attribute.
...
- [Axes reference](/concepts/axes): the runtime model for tuples and attribute composition.
- [Integrations](/guides/integrations): zero-code paths for Tailwind and CSS-in-JS.
