...
Only if the library consumes CSS variables natively. Tailwind v4 does (see the [Tailwind integration](/guides/integrations/tailwind)); any provider whose theme values are `var(--…)` strings does (see the [CSS-in-JS integration](/guides/integrations/css-in-js)). If the theme is a JS object _read_ at render time (MUI's `createTheme`, Chakra, Mantine), use `useActiveAxes()` in React, or the DOM-observation path elsewhere, to rebuild the theme object on axis flips.
