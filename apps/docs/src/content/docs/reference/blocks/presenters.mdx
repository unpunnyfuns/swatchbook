...
A presenter renders one fully-realised DTCG token: no project, no graph, no listing. Each per-type block in [Overview blocks](/reference/blocks/overview#per-type) renders its visual through one, and every built-in is an exported component you can [embed directly](#embedding-directly). The registry is what lets a consumer swap one `$type`'s visual, a custom color chip say, and have every built-in block that displays a color pick it up, without touching `ColorPalette` or `TokenNavigator` internals.
...
`MotionSpeed` is the union of `MotionSample`'s four allowed playback rates, re-exported for hosts that drive the speed externally.
...
export function TokenDocs(): ReactElement {
  return (
    <SwatchbookProvider snapshot={wire} defaultAxes={{ mode: 'Light' }} presenters={{ color: MyColorSwatch }}>
...
- `TypeSpecimen`, `FontFamilySpecimen`, `FontWeightSpecimen`: `sample?: string`, set by `TypographyScale` / `FontFamilyPreview` / `FontWeightScale`'s own `sample` prop.
- `OpacitySwatch`: `sampleColorVar?: string`, the color painted behind the checkerboard so the chip's alpha reads visually; defaults to the swatchbook accent surface.
