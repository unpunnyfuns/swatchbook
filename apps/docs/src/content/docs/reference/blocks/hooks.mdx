...
Returns the active color format (`'hex' | 'rgb' | 'hsl' | 'oklch' | 'raw'`) from the context/snapshot/default chain: a `ColorFormatContext` value (a host wrapping a subtree in its own provider), then the Storybook globals channel (`swatchbookColorFormat`, unset unless a story sets it directly), then `Config.defaultColorFormat` from the active project snapshot (the `SwatchbookProvider` a story decorator mounts, or, for blocks embedded directly in MDX with no `<Story/>`, the channel-fed snapshot the addon injects), then `'hex'`.

The full precedence order for a canned block also includes its own `colorFormat` prop, which sits above everything this hook resolves: `colorFormat` prop → `ColorFormatContext` → `Config.defaultColorFormat` → `'hex'`. A block's own prop always wins for that block; when unset, the block falls through to whatever `useColorFormat()` resolves. Pair with `formatColor(value, format)` (also exported) to render colors the same way the built-in blocks do. `hex` falls back to space-separated `rgb()` for out-of-gamut colors, marked with a ⚠ warning.
