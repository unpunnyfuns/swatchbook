...
Props: `path: string`, `heading?: string` (defaults to `path`), `colorFormat?: ColorFormat` (overrides the ambient `useColorFormat()` chain for this block and its composed `CompositeBreakdown` / `AxisVariance` sub-parts; see [`useColorFormat`](/reference/blocks/hooks#usecolorformat)).
