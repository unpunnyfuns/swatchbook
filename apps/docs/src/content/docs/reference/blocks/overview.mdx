...
Wide sample per `gradient` token (linear, left → right by default) with a stop list. DTCG gradients are stop arrays; the gradient _function_ is a rendering choice the consumer makes.
