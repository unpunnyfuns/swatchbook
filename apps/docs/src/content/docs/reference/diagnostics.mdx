...
`group` is stable across releases, so consumers can filter on it. Diagnostics originating inside Terrazzo (config, linting, parsing, plugin, resolver problems) pass through with Terrazzo's own bare group name: `config`, `lint`, `parser`, `plugin`, `resolver`. swatchbook's own diagnostics carry a `swatchbook/<area>` prefix, catalogued below.
...
| The listing JSON had no `data` array.                                | The plugin version. swatchbook pins `@terrazzo/plugin-token-listing` at `0.1.1`; an incompatible version change in the plugin's output shape can produce this.                     |

The first three fall back to raw values: `<TokenTable>` and `<ColorTable>` previews remain readable, but per-platform name columns are blank.

### `preview-value`

A single token's preview string came back containing `undefined`. Carries the token path in the message and `preview-value` on `label`.

The listing itself is fine here; one entry's `previewValue` is not. Blocks forward `previewValue` verbatim, so the string reaches the user as a table cell reading `undefinedundefined` unless the token is corrected.

The usual cause is a `$value` written in a form the CSS stringifier can't read: a dimension as a bare string (`"1rem"`) or number (`0`) rather than `{ value, unit }`. It reaches composites too, so a typography token's `fontSize` or `letterSpacing`, a shadow offset, or a border width in that form all produce it.
...
This wrap turns colorjs.io-style tracebacks (`coords.map is not a function`, four frames deep in a third-party library) into a single actionable line of detail. The `swatchbook/token-graph` load-time validators catch most malformed-shape causes before they reach emit; the wrap catches the rest.
