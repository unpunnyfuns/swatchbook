...
Plain-parse (`tokens` alone) and layered (`axes` + `tokens`) projects load through Terrazzo's `parse()`, which runs the lint pass this field configures. Resolver-backed projects (`resolver` set) load through `loadResolver`, which doesn't run linting; `lintOptions` has no effect there.
