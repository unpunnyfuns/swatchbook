...
All three variants extend a shared base (`cssVarPrefix`, `presets`, `disabledAxes`, `chrome`, `indicators`, `cssOptions`, `lintOptions`, `listingOptions`, `terrazzoPlugins`, `maxJointArity`, `default`, `outDir`).
...
A block's own `colorFormat` prop overrides this, and so does wrapping a subtree in `ColorFormatContext`; see [`useColorFormat`](/reference/blocks/hooks#usecolorformat) for the full precedence order.
...
Type: Terrazzo's `Config['lint']`, optional.

Lint configuration for swatchbook's internal parse pass, in the same shape as `terrazzo.config.ts`'s `lint` field (`{ build?, rules? }`). Omitted, Terrazzo's recommended rules apply; a supplied `rules` object replaces that set rather than merging into it, so any rule left out of it does not run.

All three config shapes lint. Plain-parse and layered projects lint through Terrazzo's `parse()`; resolver-backed projects load through `loadResolver`, which does not lint, so swatchbook runs the pass itself against the baseline token set. Set `build: { enabled: false }` to skip linting entirely, matching how the Terrazzo CLI reads the same field.

Forward the production config's `lint` field rather than restating rules, so `<Diagnostics />` reports the same pass/fail state as the project's own `terrazzo build`:

```ts
// swatchbook.config.ts
import { defineSwatchbookConfig } from '@unpunnyfuns/swatchbook-core';
import terrazzo from './terrazzo.config.ts';

export default defineSwatchbookConfig({
  tokens: ['tokens/**/*.json'],
  lintOptions: terrazzo.lint,
});
```
