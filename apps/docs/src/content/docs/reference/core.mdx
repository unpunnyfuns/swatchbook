...
The framework-free foundation the rest of swatchbook builds on. It parses DTCG token files through Terrazzo, builds a walkable token graph you can query for any token's value in any axis combination, and resolves aliases per tuple. No React, no Storybook, and no need to reach past it into `@terrazzo/parser` for the common cases.
