...
import SwatchbookSwitcherDemo from '#/components/SwatchbookSwitcherDemo.tsx';
...
<SwatchbookSwitcherDemo client:load />

The `mode` axis is bridged to this site's own light/dark theme: flipping it here updates the page immediately, and flipping the theme toggle in the site header updates the switcher's `mode` pill back.
