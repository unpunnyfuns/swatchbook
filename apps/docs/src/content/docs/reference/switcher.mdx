...
| `footer`        | no       | Host-specific node rendered after the axes. Neither switcher mount (the Storybook addon toolbar, this docs site's navbar) passes one; the package exports a `<ColorFormatSelector>` a host can slot in here if it wants a color-format control. |
