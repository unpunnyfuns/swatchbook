...
The component is a _menu_, not a _button_. It draws no trigger of its own; you decide when it's visible (popover, drawer, inline) and how the active tuple gets applied.
...
The component is _pure_: it reads its inputs, draws the menu, and calls back when the user clicks. It does not:
...
Persisting across reloads is also up to the host. The Storybook addon piggybacks on Storybook's globals API; the docs-site switcher syncs with the site's own light/dark theme (Starlight's `data-theme` + `starlight-theme` `localStorage` key) for `mode` and persists the rest via a separate `localStorage` key.
