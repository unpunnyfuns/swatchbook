...
The toolbar writes `swatchbookAxes`. Blocks' starting color format comes from [`Config.defaultColorFormat`](/reference/config#defaultcolorformat) instead of a toolbar control; `hex` falls back to space-separated `rgb()` (flagged with a ⚠ warning marker) for colors outside sRGB gamut. See [`useColorFormat`](/reference/blocks/hooks#usecolorformat) for the full precedence order.
