...
This is the "someone will bleed on this" list: shapes that trip newcomers, and sometimes returnees. Each entry states the shape, the reason, and the defensive rule.
...
**Shape:** post-1.0, swatchbook treats major versions as deliberate stability commitments. Breaking changes must bump major to signal incompatibility to users.

**Rule:** breaking changes → `major`. Features → `minor`. Bug fixes → `patch`. Docs-only PRs don't need a changeset; see _Docs-only PRs don't need a changeset_ below.
...
**Shape:** blocks read nine chrome variables (surface colors, text colors, border roles, plus the body font stack) from a fixed `--swatchbook-*` namespace that's independent of the consumer's `cssVarPrefix`. `DEFAULT_CHROME_MAP` provides one hard-coded light-scheme literal per role, not a `light-dark()` value: swatchbook has no intrinsic dark axis, so the zero-config default is a single committed appearance, and `light-dark()` / system colors would couple the chrome to the OS color-scheme, which is foreign to swatchbook's axis model. `config.chrome` wires roles to consumer tokens for per-axis variation.
