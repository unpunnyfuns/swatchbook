...
the `tokens-reference` fixture (flat per-`$type` tokens with palette
primitives under `color.palette.*` and semantic roles at
`color.<role>.*`, plus `mode × brand × contrast` modifier axes via a
DTCG resolver) through the addon and dogfoods every block in the
library.
