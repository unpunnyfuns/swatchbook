...
- **Writing a changeset:** PRs with user-visible changes run `pnpm changeset` locally and commit the generated `.changeset/*.md`. Internal-only refactors can skip it. **Docs-only PRs add a `patch` changeset** so `scripts/snapshot-docs-version.mjs` re-cuts the archived stable snapshot on release — without it, the docs fix only reaches root (the unreleased current tree), not the archived version. **Keep the summary to one line:** only the changeset's first line lands in the changelog (the `.changeset/changelog-terse.cjs` formatter keeps the headline and drops the rest), so put any detail in the PR or commit, not the changeset body.
...
- **Docs versioning:** the Astro docs site uses the `starlight-versions` Starlight plugin, not Docusaurus's built-in versioning. The plugin archives whatever's currently in `apps/docs/src/content/docs/**` into `apps/docs/src/content/docs/<slug>/**` (plus a `apps/docs/src/content/versions/<slug>.json` sidebar manifest) the moment a new slug is added to `apps/docs/src/versions.config.json` and a docs build runs — it has no CLI, no "replace a version" operation, and only ever adds one new slug at a time. `scripts/snapshot-docs-version.mjs` runs as part of the release: it deletes the previously archived slug's content + manifest, points `versions.config.json` at the newly released `<minor>` series, then runs one docs build so the plugin performs the archive. Steady state is inverted from the old Docusaurus layout: the plugin always serves "current" unprefixed at root and archived versions under `/<slug>/`, so root `/` is the unreleased next tree and the archived stable release lives at `/<minor>/` (there is no plugin config to swap that). A custom `Banner` override (`apps/docs/src/components/CurrentVersionBanner.astro`) marks root pages as unreleased, since the plugin's own banner only warns on archived pages.
