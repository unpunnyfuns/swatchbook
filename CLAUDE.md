...
`v1.0.0 shipped` (stable, on the npm `latest` dist-tag). Six published packages in a fixed-version Changesets group — `core`, `addon`, `blocks`, `switcher`, `integrations`, `mcp` — plus private workspaces (`tokens`, `apps/docs`, `apps/storybook`). Documentation site at https://unpunnyfuns.github.io/swatchbook/ built with Astro + Starlight, search via Pagefind, output to `apps/docs/dist/`. A single current docs set lives under `apps/docs/src/content/docs/**` and serves at root — no versioned snapshots, no `/next/` split; every deploy rebuilds the whole site from the docs on `main`. Post-1.0; breaking changes take a **major** bump.
...
- **Terrazzo alignment surface** on `Config`: `cssOptions`, `lintOptions`, `listingOptions`, `terrazzoPlugins`. Forward into the internal Terrazzo build so a single shared options module keeps consumer's production CLI and our preview build in lockstep. `cssOptions` strips `variableName` / `permutations` / `filename` / `skipBuild` (load-bearing); `swatchbook/css-options` warn diagnostic for the soft-inert `baseSelector` / `baseScheme` / `modeSelectors` trio. `lintOptions` forwards the consumer's Terrazzo `lint` config into swatchbook's internal parse so `<Diagnostics />` matches `terrazzo build`; omitted, Terrazzo's recommended rules apply. `@terrazzo/parser` and `@terrazzo/plugin-css` are declared as peer deps on core to enforce single-shared-instance via pnpm hoist.
...
- **Block-chrome config** stays accurate: blocks read nine roles from a fixed `--swatchbook-*` namespace independent of `cssVarPrefix`; `DEFAULT_CHROME_MAP` uses hard-coded light-scheme literals (no `light-dark()`, no system colors — swatchbook has no intrinsic dark axis, and coupling to OS color-scheme is foreign to its axis model). `config.chrome = { surfaceDefault: 'color.brand.bg.primary', … }` wires individual roles to consumer tokens.
...
- **Storybook version support:** dev deps track latest (per the deps policy above); `addon`'s **peer** range declares the supported window, currently `^10.1.0` (all of Storybook 10.x). `blocks` carries no storybook dependency at all since the `registerChannel` decoupling, so it doesn't constrain the window. **A new Storybook major is a swatchbook minor when nothing of ours breaks.** Widening the peer to `^10.1.0 || ^11.0.0` is additive: no consumer's install stops resolving, so it doesn't earn a major. Reserve the major for the case where supporting the new Storybook forces a breaking change on swatchbook's own surface, or where the old major has to be dropped from the range. Support is what the test surface proves, not what the calendar says: widen only once the new major is GA and the `storybook-compat` entry for it is green. (A caret range doesn't match prereleases, so `^11.0.0` excludes `11.0.0-alpha.1` regardless.) The `storybook-compat` CI job re-pins the lockstep `@storybook/*` per matrix entry and re-runs the version-sensitive surface: a `~10.1.0` entry guarding the floor and a `next` entry as a forward canary for the coming major. The `build` job covers latest. `@storybook/addon-mcp` carries its own per-entry spec, since it only joined the lockstep line at 10.6.0 and has no release on the floor. Keep `storybook/internal/*` imports minimal and centralized so the next-major break surface stays small.
...
- **Package manager:** pnpm@11.1.2 (workspaces); Turborepo orchestration. `pnpm-workspace.yaml`'s `overrides:` pins a handful of transitive deps to clear GHSA advisories that the natural dependency range doesn't reach on its own — each entry documents the advisory and the consumer chain inline; verify with `pnpm audit` before adding or removing one.
...
Four navbar pills: Guides / Reference / Concepts / Developers. Reference sidebar groups two categories (Packages / Blocks) so the index reads as an index, not a flat list. Integrations live under `guides/integrations/` (one index + one page per integration). Concepts holds the mental-model pages (overview, axes, token pipeline) as its own top-level section. Markdown links between docs are **extensionless, root-absolute slugs** — `/reference/config`, not `/reference/config.mdx` and not a relative path. Astro doesn't base-prefix markdown-authored links itself (only the nav/sidebar it generates), so `rehypeBaseLinks` (`apps/docs/src/plugins/rehype-base-links.mjs`) rewrites them to carry the `/swatchbook` base at build time; write links unprefixed and let the plugin handle it.
...
- **Storybook-major checklist** (per the Storybook version support policy above — a swatchbook minor unless something of ours breaks): the version-fragile surface is small and pinned. `storybook/internal/*` imports (unstable across majors) are exactly two — `internal/components` in `manager.tsx` and `internal/csf` in `index.ts` — guarded by `addon/test/storybook-internal-surface.test.ts` (adding one fails the test). The public-but-still-checkable surface is `storybook/manager-api` (manager) and `storybook/preview-api` (preview + blocks). On a Storybook major: re-verify those import sites, confirm the `storybook-compat` entry for the new major is green, then widen the peer ranges to `^<old> || ^<new>` and pin the matrix entry to the GA spec. Widening is a minor; only a forced break on our own surface, or dropping the old major from the range, makes it a major. Both ends of the supported range are exercised by the `storybook-compat` CI job.
...
- **Writing a changeset:** PRs with user-visible changes run `pnpm changeset` locally and commit the generated `.changeset/*.md`. Internal-only refactors can skip it. Docs-only PRs don't need one either — there's no snapshot to rebuild; a docs fix on `main` reaches the live site on the next deploy. **Keep the summary to one line:** only the changeset's first line lands in the changelog (the `.changeset/changelog-terse.cjs` formatter keeps the headline and drops the rest), so put any detail in the PR or commit, not the changeset body.
...
- **Docs versioning:** dropped. The Astro docs site serves a single current set from `apps/docs/src/content/docs/**` at root; there's no per-release snapshot, no `/next/` split, and no versioning plugin. A release doesn't touch `apps/docs/` at all — the docs deploy workflow (`.github/workflows/docs.yml`) rebuilds straight from `main` on every push, independent of the package release cadence.
